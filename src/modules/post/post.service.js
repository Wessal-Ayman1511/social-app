...
export const getPosts = async (req, res, next) => {
  ...
  const totalPages = Math.ceil(parseFloat(totalPosts / size));

  if (!totalPages)
    return next(new Error(messages.post.notFound, { cause: 404 }));
  return res
    .status(200)
    .json({ success: true, data: { posts, currentPage, totalPages } });
...
export const getSpecificPost = async (req, res, next) => {
  ...
  return post
    ...
    : next(new Error(messages.post.notFound, { cause: 404 }));
};
